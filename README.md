...
4. **简化字处理**：对于「后/後」「谷/穀」「丰/豐」「离/離」等《说文》中本属不同字头、意义各异的简化合并字，仅收录正体/本字。若对应简化字本身亦有收录价值且其字形来源独立，则另行收录，不与本字共用同一字形。例如，「后」虽为「後」的简化合并字，但「王后」之「后」在《说文》中自有字头，且与「皇」「帝」「公」「卿」等同为政治身份类常用字，故单独收录。
